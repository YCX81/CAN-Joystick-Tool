...
bool verifyMiniJoystickExtensions(const QJsonObject &cross2d)
{
    QJsonObject config = cross2d;
    QJsonObject product = config.value(QStringLiteral("product")).toObject();
    product.insert(QStringLiteral("customerBindings"),
                   QJsonArray{QJsonObject{
                       {QStringLiteral("name"), QStringLiteral("湖南中联重科电动叉车")},
    ...
    QJsonArray cards = layout.value(QStringLiteral("cards")).toArray();
    for (qsizetype cardIndex = 0; cardIndex < cards.size(); ++cardIndex) {
        QJsonObject card = cards.at(cardIndex).toObject();
        if (card.value(QStringLiteral("kind")).toString() != QStringLiteral("controls"))
            continue;
        QJsonArray elements = card.value(QStringLiteral("elements")).toArray();
        if (elements.isEmpty())
            continue;
        QJsonObject element = elements.first().toObject();
        element.insert(QStringLiteral("renderer"), QStringLiteral("miniJoystick"));
        element.insert(QStringLiteral("properties"),
                       QJsonObject{
                           {QStringLiteral("invertX"), true},
                           {QStringLiteral("invertY"), false},
                           {QStringLiteral("testPattern"), QStringLiteral("cross")},
                           {QStringLiteral("cycleCount"), 3}
                        });
        elements[0] = element;
        card.insert(QStringLiteral("elements"), elements);
    ...
    bool ok = expectValid(config, QStringLiteral("C0031 mini joystick extensions"));

    QJsonObject invalid = config;
    layout = invalid.value(QStringLiteral("layout")).toObject();
    cards = layout.value(QStringLiteral("cards")).toArray();
    for (qsizetype cardIndex = 0; cardIndex < cards.size(); ++cardIndex) {
        QJsonObject card = cards.at(cardIndex).toObject();
        QJsonArray elements = card.value(QStringLiteral("elements")).toArray();
        if (elements.isEmpty()
            || elements.first().toObject().value(QStringLiteral("renderer")).toString()
                   != QStringLiteral("miniJoystick")) {
            continue;
        }
        QJsonObject element = elements.first().toObject();
        QJsonObject properties = element.value(QStringLiteral("properties")).toObject();
        properties.insert(QStringLiteral("testPattern"), QStringLiteral("circle"));
        element.insert(QStringLiteral("properties"), properties);
        elements[0] = element;
        card.insert(QStringLiteral("elements"), elements);
        cards[cardIndex] = card;
    ...
    layout.insert(QStringLiteral("cards"), cards);
    invalid.insert(QStringLiteral("layout"), layout);
